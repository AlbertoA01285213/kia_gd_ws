...
def generate_launch_description():
    ...
    # Archivo para recibir poses del robot
    ur_rtde_driver_pkg = get_package_share_directory('ur_ros_rtde')
    ur_rtde_recieve_launch = os.path.join(
        ur_rtde_driver_pkg,
        'launch',
        'robot_state_receiver.launch.py'
    )

    # Archivo para mandar poses al robot
    ur_rtde_send_launch = os.path.join(
        ur_rtde_driver_pkg,
        'launch',
    ...
    return LaunchDescription([
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(ur_rtde_recieve_launch),
            launch_arguments = {"robot_ip": target_ip}.items()
        ),

        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(ur_rtde_send_launch),
            launch_arguments = {"robot_ip": target_ip}.items()
